...
- **Telegram-бот (кнопка Mini App)**
  - `TELEGRAM_BOT_TOKEN` — тот же токен, что и для backend (бот при /start отправляет кнопку «Играть» с `web_app`).
  - `MINI_APP_URL` — URL Mini App для кнопки (по умолчанию `https://app.games-telegram.online`). Должен быть HTTPS; только при открытии по этой кнопке Telegram передаёт `initData` в приложение.
...
| `npm run telegram-bot` | Запуск Telegram-бота: при /start отправляет кнопку «Играть» с Web App. Требует `TELEGRAM_BOT_TOKEN` и опционально `MINI_APP_URL` (по умолчанию `https://app.games-telegram.online`). |
